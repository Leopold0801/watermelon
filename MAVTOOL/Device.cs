...
namespace MAVTOOL
{
    public delegate void readmavlink(MAVState MAV,bool IsCom, string Ip);//声明一个委托
    ...
    public class Device
    {
        ...
        public bool IsConnect
        {
            get
            {
                if (Environment.TickCount - _comPort.MAV.cs.LastTime < 3000)
                {
                    return true;
                }
                else
                ...
                if ((comPort.BaseStream.PortName != null) && (comPort.BaseStream.PortName != "None"))
                {
                    ...
                    try
                    {
                        ...
                        if (ConnectType == ConnectType.Tcp)
                        {
                            ...
                            comPort.IP = IP;
                            comPort.Port = Port.ToString();

                            comPort.BaseStream.IP = IP;
                            comPort.BaseStream.Port = Port;
                            comPort.MAV.cs.ResetInternals();//初始化接口
                            comPort.BaseStream.PortName = "TCP";
                        ...
                        else if (ConnectType == ConnectType.Com)
                        {
        ...
        //connect读取的一个事件
        void readcomdisplay_readmavlinkEvent(MAVState MAV,bool falg, string Ip)
        {
            ...
            if (linkEvent != null)
            {
                ...
                linkEvent.Invoke(MAV,true,ComPort);//事件触发更新UI界面
                else
                    linkEvent.Invoke(MAV, false, Ip);//
            }
        }
